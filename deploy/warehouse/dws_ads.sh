...
ads_activity_stats="
    insert overwrite table ${HIVE_DATA_BASE}.ads_activity_stats
        select * from ${HIVE_DATA_BASE}.ads_activity_stats
        union
        select '${do_date}' dt,
                activity_id,
                activity_name,
...
ads_coupon_stats="
    insert overwrite table ${HIVE_DATA_BASE}.ads_coupon_stats
        select * from ${HIVE_DATA_BASE}.ads_coupon_stats
        union
        select '${do_date}' dt,
               coupon_id,
               coupon_name,
...
ads_new_buyer_stats="
    insert overwrite table ${HIVE_DATA_BASE}.ads_new_buyer_stats
        select * from ${HIVE_DATA_BASE}.ads_new_buyer_stats
        union
        select '${do_date}',
               odr.recent_days,
...
ads_order_by_province="
    insert overwrite table ${HIVE_DATA_BASE}.ads_order_by_province
        select * from ${HIVE_DATA_BASE}.ads_order_by_province
        union
        select '${do_date}'    dt,
               1,
               province_id,
        ...
        from ${HIVE_DATA_BASE}.dws_trade_province_order_nd
...
ads_page_path="
    insert overwrite table ${HIVE_DATA_BASE}.ads_page_path
        select * from ${HIVE_DATA_BASE}.ads_page_path
        union
        select '${do_date}'          dt,
                recent_days,
                source,
        ...
        (
            ...
            (
                ...
                from ${HIVE_DATA_BASE}.dwd_traffic_page_view_inc lateral view explode(array(1, 7, 30)) tmp as recent_days
...
ads_repeat_purchase_by_tm="
    insert overwrite table ${HIVE_DATA_BASE}.ads_repeat_purchase_by_tm
        select * from ${HIVE_DATA_BASE}.ads_repeat_purchase_by_tm
        union
        select '${do_date}' dt,
               recent_days,
               tm_id,
...
ads_sku_cart_num_top3_by_cate="
    insert overwrite table ${HIVE_DATA_BASE}.ads_sku_cart_num_top3_by_cate
        select * from ${HIVE_DATA_BASE}.ads_sku_cart_num_top3_by_cate
        union
        select '${do_date}'     dt,
                category1_id,
                category1_name,
        ...
        (
            ...
            (
                ...
                from ${HIVE_DATA_BASE}.dwd_trade_cart_full
                where dt = '${do_date}'
                group by sku_id
            ) cart left join
            (
...
ads_trade_stats="
    insert overwrite table ${HIVE_DATA_BASE}.ads_trade_stats
        select * from ${HIVE_DATA_BASE}.ads_trade_stats
        union
        select '${do_date}',
               odr.recent_days,
               order_total_amount,
...
ads_trade_stats_by_cate="
    insert overwrite table ${HIVE_DATA_BASE}.ads_trade_stats_by_cate
        select * from ${HIVE_DATA_BASE}.ads_trade_stats_by_cate
        union
        select '${do_date}' dt,
...
ads_trade_stats_by_tm="
    insert overwrite table ${HIVE_DATA_BASE}.ads_trade_stats_by_tm
        select * from ${HIVE_DATA_BASE}.ads_trade_stats_by_tm
        union
        select '${do_date}' dt,
            nvl(odr.recent_days,         refund.recent_days),
...
ads_traffic_stats_by_channel="
    insert overwrite table ${HIVE_DATA_BASE}.ads_traffic_stats_by_channel
        select * from ${HIVE_DATA_BASE}.ads_traffic_stats_by_channel
        union
        select '${do_date}' dt,
               recent_days,
               channel,
...
ads_user_action="
    insert overwrite table ${HIVE_DATA_BASE}.ads_user_action
        select * from ${HIVE_DATA_BASE}.ads_user_action
        union
        select '${do_date}'      dt,
               page.recent_days,
               home_count,
...
ads_user_change="
    insert overwrite table ${HIVE_DATA_BASE}.ads_user_change
        select * from ${HIVE_DATA_BASE}.ads_user_change
        union
        select churn.dt,
               user_churn_count,
               user_back_count
...
ads_user_retention="
    insert overwrite table ${HIVE_DATA_BASE}.ads_user_retention
        select * from ${HIVE_DATA_BASE}.ads_user_retention
        union
        select '${do_date}' dt,
        ...
        (
            ...
            from ${HIVE_DATA_BASE}.dwd_user_register_inc
...
ads_user_stats="
    insert overwrite table ${HIVE_DATA_BASE}.ads_user_stats
        select * from ${HIVE_DATA_BASE}.ads_user_stats
        union
        select '${do_date}' dt,
            t1.recent_days,
            new_user_count,
        ...
        (
            select recent_days,
                   count(*)    new_user_count
            from ${HIVE_DATA_BASE}.dwd_user_register_inc lateral view explode(array(1, 7, 30)) tmp as recent_days
            where dt >= date_add('${do_date}', -recent_days + 1)
            group by recent_days
        ) t1 join 
        (
