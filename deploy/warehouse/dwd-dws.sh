...
#    Description   ：  dwd-dws.sh 被用于 ==> 将 DWD 层数据加载到 DWS 
# =========================================================================================
...
LOG_FILE="dwd-dws-$(date +%F).log"                         # 执行日志
...
dws_trade_user_sku_order_1d="
    insert overwrite table ${HIVE_DATA_BASE}.dws_trade_user_sku_order_1d partition (dt = '${do_date}')
...
dws_trade_user_sku_order_refund_1d="
    insert overwrite table ${HIVE_DATA_BASE}.dws_trade_user_sku_order_refund_1d partition (dt = '${do_date}')
...
dws_trade_user_order_1d="
    insert overwrite table ${HIVE_DATA_BASE}.dws_trade_user_order_1d partition (dt = '${do_date}')
    select user_id,
...
dws_trade_user_cart_add_1d="
    insert overwrite table ${HIVE_DATA_BASE}.dws_trade_user_cart_add_1d partition (dt = '${do_date}')
    select user_id, 
...
dws_trade_user_payment_1d="
    insert overwrite table ${HIVE_DATA_BASE}.dws_trade_user_payment_1d partition (dt = '${do_date}')
    select user_id,
...
dws_trade_user_order_refund_1d="
    insert overwrite table ${HIVE_DATA_BASE}.dws_trade_user_order_refund_1d partition (dt = '${do_date}')
    select user_id, 
           count(*)           as order_refund_count,
           sum(refund_num)    as order_refund_num,
           sum(refund_amount) as order_refund_amount
...
dws_trade_province_order_1d="
    insert overwrite table ${HIVE_DATA_BASE}.dws_trade_province_order_1d partition (dt = '${do_date}')
...
dws_trade_user_order_td="
	...
	select nvl(old.user_id, new.user_id)                                                 as user_id,
		   if(old.user_id is null,     '${do_date}', old.order_date_first)               as order_date_first,
		   if(new.user_id is not null, '${do_date}', old.order_date_last)                as order_date_last,
	...
	(
		select user_id,
			   ...
			   total_amount_td
		from ${HIVE_DATA_BASE}.dws_trade_user_order_td
...
dws_trade_user_payment_td="
	...
	select nvl(old.user_id, new.user_id)                                     as user_id,
		   if(old.user_id is null,     '${do_date}', old.payment_date_first) as payment_date_first,
		   if(new.user_id is not null, '${do_date}', old.payment_date_last)  as payment_date_last,
	...
	(
		select user_id, 
			   ...
			   payment_num_td, 
			   payment_amount_td
		from ${HIVE_DATA_BASE}.dws_trade_user_payment_td
		where dt = date_add('${do_date}', -1)
	) as old full outer join 
	(
		select user_id, 
